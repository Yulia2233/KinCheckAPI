...
function updateMetrics() {
  ...
  document.querySelector("#input-speed").textContent = input === null ? "--" : `${input.toFixed(3)} rad/s`;
  document.querySelector("#output-speed").textContent = output === null ? "--" : `${output.toFixed(3)} rad/s`;
  document.querySelector("#ratio-value").textContent =
    input === null || output === null || Math.abs(output) < 1e-10 ? "--" : `${Math.abs(input / output).toFixed(3)}:1`;
}
...
async function initialize() {
  try {
    ...
    document.querySelector("#expected-ratio").textContent =
      manifest.metrics.expected_ratio === null ? "--" : `${manifest.metrics.expected_ratio.toFixed(3)}:1`;
