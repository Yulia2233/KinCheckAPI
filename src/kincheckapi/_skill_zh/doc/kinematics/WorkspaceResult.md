...
表示 `WorkspaceResult` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `target` | `TargetReference` | 必填 | `target` 的公开输入或数据字段。 |
| `samples` | `tuple[WorkspaceSample, ...]` | 必填 | `samples` 的公开输入或数据字段。 |
| `reachable_points` | `tuple[Pose, ...]` | 必填 | `reachable_points` 的公开输入或数据字段。 |
| `bounds_m` | `Mapping[str, tuple[float, float]]` | 必填 | `bounds_m`，单位 m，必须为有限值。 |
| `reachable_fraction` | `float` | 必填 | `reachable_fraction` 的公开输入或数据字段。 |
