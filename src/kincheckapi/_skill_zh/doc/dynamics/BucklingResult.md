...
表示 `BucklingResult` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `operation` | `str` | `'solve_buckling_screening'` | `operation` 的公开输入或数据字段。 |
...
| `eigenvalues` | `tuple[float, ...]` | `()` | `eigenvalues` 的公开输入或数据字段。 |
| `mode_shapes` | `tuple[tuple[float, ...], ...]` | `()` | `mode_shapes` 的公开输入或数据字段。 |
| `reference_load_n` | `float` | `0.0` | `reference_load_n` 的公开输入或数据字段。 |
