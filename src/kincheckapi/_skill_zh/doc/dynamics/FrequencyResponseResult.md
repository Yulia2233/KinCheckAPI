...
表示 `FrequencyResponseResult` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `operation` | `str` | `'solve_frequency_response'` | `operation` 的公开输入或数据字段。 |
...
| `response` | `tuple[tuple[complex, ...], ...]` | `()` | `response` 的公开输入或数据字段。 |
| `peak_amplitude` | `Mapping[str, float]` | default_factory | `peak_amplitude` 的公开输入或数据字段。 |
| `damping` | `kincheckapi.vibration.DampingSpec | None` | `None` | `damping` 的公开输入或数据字段。 |
