...
表示 `StructuralLoad` 的公开、可序列化数据结构。
...
| `values` | `tuple[float, ...]` | 必填 | `values` 的公开输入或数据字段。 |
| `target_dofs` | `tuple[int, ...]` | `()` | `target_dofs` 的公开输入或数据字段。 |
| `source` | `str` | `'explicit'` | `source` 的公开输入或数据字段。 |
| `time_s` | `float | None` | `None` | 查询时间，单位 s，必须位于结果时间范围内。 |
