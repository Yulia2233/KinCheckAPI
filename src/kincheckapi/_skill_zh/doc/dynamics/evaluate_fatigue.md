...
执行公开操作 `evaluate_fatigue`。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `history` | `kincheckapi.fatigue.StressHistory` | 必填 | `history` 的公开输入或数据字段。 |
| `material` | `kincheckapi.fatigue.FatigueMaterial` | 必填 | `material` 的公开输入或数据字段。 |
...
| `repeat_count` | `int` | `1` | `repeat_count` 的公开输入或数据字段。 |
