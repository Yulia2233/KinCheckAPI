...
表示 `ElasticMaterial` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `material_id` | `str` | 必填 | 稳定且可解析的 `material_id`。 |
| `youngs_modulus_pa` | `float` | 必填 | `youngs_modulus_pa` 的公开输入或数据字段。 |
| `poisson_ratio` | `float` | 必填 | `poisson_ratio` 的公开输入或数据字段。 |
| `density_kg_m3` | `float` | 必填 | `density_kg_m3` 的公开输入或数据字段。 |
| `source` | `str` | 必填 | `source` 的公开输入或数据字段。 |
| `yield_strength_pa` | `float | None` | `None` | `yield_strength_pa` 的公开输入或数据字段。 |
| `ultimate_strength_pa` | `float | None` | `None` | `ultimate_strength_pa` 的公开输入或数据字段。 |
