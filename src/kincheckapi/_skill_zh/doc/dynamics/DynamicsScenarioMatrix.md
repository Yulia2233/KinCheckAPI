...
表示 `DynamicsScenarioMatrix` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `matrix_id` | `str` | 必填 | 稳定且可解析的 `matrix_id`。 |
| `cases` | `tuple[kincheckapi.dynamics_v07.DynamicsScenarioCase, ...]` | 必填 | `cases` 的公开输入或数据字段。 |
| `requested_case_ids` | `tuple[str, ...] | None` | `None` | 显式指定的 `requested_case_ids` 集合。 |
