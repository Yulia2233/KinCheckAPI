...
表示 `FatigueReport` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `operation` | `str` | `'evaluate_fatigue'` | `operation` 的公开输入或数据字段。 |
...
| `correction` | `str` | `'none'` | `correction` 的公开输入或数据字段。 |
| `residual_indices` | `tuple[int, ...]` | `()` | `residual_indices` 的公开输入或数据字段。 |
