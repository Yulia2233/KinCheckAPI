...
表示 `StructuralResult` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `operation` | `str` | `'solve_static_structure'` | `operation` 的公开输入或数据字段。 |
...
| `displacements_m` | `Mapping[str, float]` | default_factory | `displacements_m`，单位 m，必须为有限值。 |
| `reactions_n` | `Mapping[str, float]` | default_factory | `reactions_n` 的公开输入或数据字段。 |
| `stresses_pa` | `Mapping[str, float]` | default_factory | `stresses_pa` 的公开输入或数据字段。 |
| `strains` | `Mapping[str, float]` | default_factory | `strains` 的公开输入或数据字段。 |
| `load_resultant` | `tuple[float, ...]` | `()` | `load_resultant` 的公开输入或数据字段。 |
| `residual_norm` | `float` | `0.0` | `residual_norm` 的公开输入或数据字段。 |
| `converged` | `bool` | `False` | `converged` 的公开输入或数据字段。 |
| `roi` | `tuple[str, ...]` | `()` | `roi` 的公开输入或数据字段。 |
