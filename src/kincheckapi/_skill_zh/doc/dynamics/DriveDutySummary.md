...
表示 `DriveDutySummary` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `operation` | `str` | `'summarize_drive_duty'` | `operation` 的公开输入或数据字段。 |
...
| `torque_peak_nm` | `float` | `0.0` | `torque_peak_nm` 的公开输入或数据字段。 |
| `torque_rms_nm` | `float` | `0.0` | `torque_rms_nm` 的公开输入或数据字段。 |
| `speed_peak_rad_s` | `float` | `0.0` | `speed_peak_rad_s`，单位 rad/s，必须为有限值。 |
| `power_peak_w` | `float` | `0.0` | `power_peak_w` 的公开输入或数据字段。 |
| `energy_positive_j` | `float` | `0.0` | `energy_positive_j` 的公开输入或数据字段。 |
| `energy_negative_j` | `float` | `0.0` | `energy_negative_j` 的公开输入或数据字段。 |
| `energy_net_j` | `float` | `0.0` | `energy_net_j` 的公开输入或数据字段。 |
