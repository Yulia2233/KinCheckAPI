...
表示 `RandomLoadSpec` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `frequency_hz` | `tuple[float, ...]` | 必填 | `frequency_hz` 的公开输入或数据字段。 |
| `psd` | `tuple[float, ...]` | 必填 | `psd` 的公开输入或数据字段。 |
| `unit` | `str` | 必填 | `unit` 的公开输入或数据字段。 |
| `seed` | `int | None` | `None` | `seed` 的公开输入或数据字段。 |
| `one_sided` | `bool` | `True` | `one_sided` 的公开输入或数据字段。 |
