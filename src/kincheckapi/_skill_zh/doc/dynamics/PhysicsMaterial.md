...
表示 `PhysicsMaterial` 的公开、可序列化数据结构。
...
| `density` | `float` | 必填 | `density` 的公开输入或数据字段。 |
| `density_unit` | `str` | 必填 | `density_unit` 的公开输入或数据字段。 |
| `source` | `str` | 必填 | `source` 的公开输入或数据字段。 |
| `data_quality` | `str` | `'illustrative'` | `data_quality` 的公开输入或数据字段。 |
