...
表示 `DampingSpec` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `modal_ratios` | `tuple[float, ...]` | `()` | `modal_ratios` 的公开输入或数据字段。 |
| `rayleigh_alpha_s` | `float | None` | `None` | `rayleigh_alpha_s`，单位 s，必须为有限值。 |
| `rayleigh_beta_s` | `float | None` | `None` | `rayleigh_beta_s`，单位 s，必须为有限值。 |
