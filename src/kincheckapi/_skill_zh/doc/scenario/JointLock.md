...
```python
from kincheckapi.scenario import JointLock
```

## 用途

表示 `JointLock` 的公开、可序列化数据结构。
...
| `position_rad_or_m` | `float | None` | `None` | `position_rad_or_m`，单位 m，必须为有限值。 |
