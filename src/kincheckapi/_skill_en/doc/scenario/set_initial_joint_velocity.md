...
```python
set_initial_joint_velocity(*, scenario: Scenario, joint_id: str, velocity_rad_s_or_m_s: float) -> Scenario
```
...
```python
from kincheckapi.scenario import set_initial_joint_velocity
```

## Purpose

Set a field and return the updated immutable object: `set_initial_joint_velocity`.
...
| `velocity_rad_s_or_m_s` | `float` | required | `velocity_rad_s_or_m_s` in m/s; finite. |
