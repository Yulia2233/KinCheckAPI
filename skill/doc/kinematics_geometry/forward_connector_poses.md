...
```python
forward_connector_poses(assembly: AssemblyModel, joint_positions: Mapping[str, float]) -> Mapping[tuple[str, str], Pose]
```
...
```python
from kincheckapi.kinematics_geometry import forward_connector_poses
```

## Purpose

Propagate world poses for all connectors from the assembly tree and joint positions.
...
Returns `Mapping[tuple[str, str], Pose]`.
