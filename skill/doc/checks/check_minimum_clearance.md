...
```python
check_minimum_clearance(*, assembly: AssemblyModel, motion_result: MotionResult, check_id: str = 'minimum_clearance', **parameters: Any) -> CheckReport
```
...
```python
from kincheckapi.checks import check_minimum_clearance
```
...
| `check_id` | `str` | `'minimum_clearance'` | Stable caller-provided check ID for result traceability. |
