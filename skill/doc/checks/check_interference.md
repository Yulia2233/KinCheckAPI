...
```python
check_interference(*, assembly: AssemblyModel, motion_result: MotionResult, check_id: str = 'interference', **parameters: Any) -> CheckReport
```
...
```python
from kincheckapi.checks import check_interference
```
...
| `check_id` | `str` | `'interference'` | Stable caller-provided check ID for result traceability. |
