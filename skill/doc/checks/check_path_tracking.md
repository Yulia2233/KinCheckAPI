...
| `path_target` | `Any` | required | Public input or data field `path_target`. |
| `position_tolerance_m` | `float` | `1e-06` | `position_tolerance_m` in metres; finite. |
| `curvature_tolerance_1_m` | `float | None` | `None` | `curvature_tolerance_1_m` in metres; finite. |
| `minimum_turn_radius_m` | `float | None` | `None` | `minimum_turn_radius_m` in metres; finite. |
| `check_id` | `str` | `'path_tracking'` | Stable caller-provided check ID for result traceability. |
