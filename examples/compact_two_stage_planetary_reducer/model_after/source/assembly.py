...
if __package__:
    ...
    from .materials import (
        make_reducer_material_rmaterial,
        make_reducer_materials_rdict,
    )
...
def _assemble_two_stage_planetary_reducer(
    *,
    ...
    """Assemble already-built durable definitions into the reducer."""
...
def make_two_stage_planetary_reducer_rassembly() -> scad.Assembly:
    """Build the full 20:1 reducer as a runtime-only assembly."""
...
def _durable_part(
    *, definition_id: str, factory: Callable[[], scad.Part]
    ...
    """Build one reusable ProductPackage Part definition."""
...
def _bearing_component_part(*, role: str) -> scad.Part:
    ...
    return scad.assign_material_rpart(
        part=bearing.get_component(component_id).item,
        material=make_reducer_material_rmaterial(key="bearing"),
    )


def _durable_bearing() -> scad.AssemblyBuildResult:
    """Build the bearing once as a reusable nested assembly definition."""
...
def build_two_stage_planetary_reducer_product() -> scad.AssemblyBuildResult:
    """Build the canonical `.scadpkg` root definition and its full closure."""
