...
def make_pivot_bolt_solid(shank_length: float, head_base_z: float, seat_z: float) -> scad.Solid:
    """One hex-head pivot bolt along Z: head at top, shank through -Z."""

    head = _hex_prism(
        radius=PIVOT_BOLT_HEAD_RADIUS,
        height=PIVOT_BOLT_HEAD_THICKNESS,
        bottom_z=head_base_z,
    )
    shank = scad.make_cylinder_rsolid(
        radius=PIVOT_BOLT_SHAFT_RADIUS,
        height=shank_length,
        bottom_face_center=(0.0, 0.0, head_base_z - shank_length),
    )
    shoulder = scad.make_cylinder_rsolid(
        radius=PIVOT_BOLT_HEAD_RADIUS * 0.8,
        height=0.6,
        bottom_face_center=(0.0, 0.0, head_base_z - 0.3),
    )
    # ---- feature: host-seating-shoulder (add, profile=geometry) ----
    # Seats on the stationary host face; the moving link clears both collars.
    seat = scad.make_cylinder_rsolid(
        radius=PIVOT_BOLT_HEAD_RADIUS * 0.8, height=0.6,
        bottom_face_center=(0.0, 0.0, seat_z),
    )
    body = scad.union_rsolid([head, shank, shoulder, seat])
...
def make_pivot_bolt_part(
    ...
    *,
    ...
    head_base_z: float = 0.0,
    seat_z: float = 0.0,
) -> scad.Part:
    """One bolt Part with a Z-axis connector at the head center."""

    body = make_pivot_bolt_solid(shank_length, head_base_z, seat_z)
