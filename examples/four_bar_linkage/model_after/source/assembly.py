...
bar at its pivot so it shares that bar's rigid group. Ground and coupler plates
use separate axial layers, while the moving-pivot bolts span the moving-link
stack.
"""
...
from dimensions import (
    ...
    GROUND_PIVOT_BOLT_SHANK_LENGTH,
...
from collision_policy import COLLISION_EXCLUSION_COMPONENT_PAIRS

CACHE = scad.CachePolicy(root=Path(__file__).resolve().parent / ".cache")
...
def _durable_parts():
    ...
    def build_ground() -> scad.Part:
        return make_link_part(
            "ground_span",
            GROUND_LENGTH,
            geometry_z_offset=GROUND_GEOMETRY_Z_OFFSET,
            material=MATERIAL,
        )
    ...
    def build_coupler() -> scad.Part:
        return make_link_part(
            "coupler",
            COUPLER_LENGTH,
            geometry_z_offset=COUPLER_GEOMETRY_Z_OFFSET,
            material=MATERIAL,
        )
    ...
    @scad.part(id="ground_pivot_bolt", cache=CACHE, project_root=Path(__file__).parent)
    def build_ground_bolt() -> scad.Part:
        return make_pivot_bolt_part(
            "ground_pivot_bolt",
            GROUND_PIVOT_BOLT_SHANK_LENGTH,
            axis_connector_z=-GROUND_GEOMETRY_Z_OFFSET,
            material=BOLT_MATERIAL,
        )

    @scad.part(id="pivot_bolt", cache=CACHE, project_root=Path(__file__).parent)
    def build_moving_bolt() -> scad.Part:
        return make_pivot_bolt_part(
            "pivot_bolt",
            PIVOT_BOLT_SHANK_LENGTH,
            material=BOLT_MATERIAL,
...
def build_four_bar_linkage() -> scad.AssemblyBuildResult:
    ...
    ground_r, crank_r, coupler_r, rocker_r, ground_bolt_r, moving_bolt_r = _durable_parts()
    ...
    def build() -> scad.Assembly:
        ...
        # not a kinematic body). Ground and moving pivots use separate bolt
        # definitions so each pin spans only its intended axial stack.
        bolt_joints = (
            ("bolt_a", "ground", (0.0, 0.0), "pivot_a", ground_bolt_r),
            ("bolt_d", "ground", ROCKER_PIVOT, "pivot_b", ground_bolt_r),
            ("bolt_b", "crank", (CRANK_LENGTH, 0.0), "pivot_b", moving_bolt_r),
        )
        for bolt_id, host_id, host_local_pivot, host_connector, bolt_result in bolt_joints:
            host_placement = next(
            ...
            bolt_world = scad.make_placement_rplacement(
                origin=(
                    ...
                    GROUND_GEOMETRY_Z_OFFSET
                    if bolt_result is ground_bolt_r
                    else 0.0,
                )
            )
            assembly = scad.add_component_rassembly(
                assembly=assembly,
                item=bolt_result.part,
