...
表示 `JointTrajectory` 的公开、可序列化数据结构。
...
| `positions` | `tuple[float, ...]` | 必填 | `positions` 的公开输入或数据字段。 |
| `velocities` | `tuple[float, ...]` | 必填 | `velocities` 的公开输入或数据字段。 |
| `accelerations` | `tuple[float, ...]` | 必填 | `accelerations` 的公开输入或数据字段。 |
