...
求解指定运动学问题：`solve_frequency_response`。
...
| `frequencies_hz` | `Sequence[float]` | 必填 | `frequencies_hz` 的公开输入或数据字段。 |
| `force_vector` | `Sequence[float]` | 必填 | `force_vector` 的公开输入或数据字段。 |
