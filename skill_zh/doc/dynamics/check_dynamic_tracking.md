...
执行结构化检查：`check_dynamic_tracking`。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `result` | `kincheckapi.dynamic_types.ForwardDynamicsResult` | 必填 | `result` 的公开输入或数据字段。 |
| `targets` | `Mapping[str, float]` | 必填 | `targets` 的公开输入或数据字段。 |
| `tolerance` | `Union[Mapping[str, float], float]` | `0.001` | `tolerance` 的公开输入或数据字段。 |
