...
表示 `ConstraintSpec` 的公开、可序列化数据结构。
...
| `coefficients` | `Mapping[str, float]` | 必填 | `coefficients` 的公开输入或数据字段。 |
| `target` | `float` | `0.0` | `target` 的公开输入或数据字段。 |
| `velocity_target` | `float` | `0.0` | `velocity_target` 的公开输入或数据字段。 |
