...
表示 `ContactDynamicsResult` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `operation` | `str` | `'solve_contact_dynamics'` | `operation` 的公开输入或数据字段。 |
...
| `events` | `tuple[kincheckapi.dynamics_v07.ContactEvent, ...]` | `()` | `events` 的公开输入或数据字段。 |
| `impulse_ns` | `float` | `0.0` | `impulse_ns` 的公开输入或数据字段。 |
| `momentum_residual_n_s` | `float` | `0.0` | `momentum_residual_n_s`，单位 s，必须为有限值。 |
| `energy_dissipation_j` | `float` | `0.0` | `energy_dissipation_j` 的公开输入或数据字段。 |
