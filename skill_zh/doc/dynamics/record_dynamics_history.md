...
执行公开操作 `record_dynamics_history`。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `result` | `kincheckapi.dynamics_v07.MultibodyResult | kincheckapi.dynamics_v07.ContactDynamicsResult` | 必填 | `result` 的公开输入或数据字段。 |
| `history_id` | `str` | `'dynamics-history'` | 稳定且可解析的 `history_id`。 |
