...
执行结构化检查：`check_contact_convergence`。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `results_by_step` | `Mapping[float, kincheckapi.dynamics_v07.ContactDynamicsResult]` | 必填 | `results_by_step` 的公开输入或数据字段。 |
| `relative_tolerance` | `float` | `0.05` | `relative_tolerance` 的公开输入或数据字段。 |
