...
表示 `GeneralizedJointState` 的公开、可序列化数据结构。
...
| `position` | `tuple[float, ...] | float` | 必填 | `position` 的公开输入或数据字段。 |
| `velocity` | `tuple[float, ...] | float` | `()` | `velocity` 的公开输入或数据字段。 |
| `acceleration` | `tuple[float, ...] | float` | `()` | `acceleration` 的公开输入或数据字段。 |
| `coordinate_frame` | `str` | `'joint'` | `coordinate_frame` 的公开输入或数据字段。 |
