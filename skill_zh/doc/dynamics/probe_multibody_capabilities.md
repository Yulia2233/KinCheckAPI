...
执行公开操作 `probe_multibody_capabilities`。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `scenario` | `kincheckapi.dynamics_v07.RigidDynamicsScenario | None` | 必填 | 已绑定装配定义的不可变 `Scenario`。 |
| `operation` | `str` | `'solve_multibody_dynamics'` | `operation` 的公开输入或数据字段。 |
