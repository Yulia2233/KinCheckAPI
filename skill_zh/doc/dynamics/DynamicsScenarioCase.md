...
表示 `DynamicsScenarioCase` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `case_id` | `str` | 必填 | 稳定且可解析的 `case_id`。 |
| `scenario` | `kincheckapi.dynamics_v07.RigidDynamicsScenario` | 必填 | 已绑定装配定义的不可变 `Scenario`。 |
| `reaction_request` | `kincheckapi.dynamics_v07.ReactionRequest | None` | `None` | `reaction_request` 的公开输入或数据字段。 |
| `tags` | `tuple[str, ...]` | `()` | `tags` 的公开输入或数据字段。 |
