...
表示 `FatigueCycle` 的公开、可序列化数据结构。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `stress_range_pa` | `float` | 必填 | `stress_range_pa` 的公开输入或数据字段。 |
| `mean_stress_pa` | `float` | 必填 | `mean_stress_pa` 的公开输入或数据字段。 |
| `count` | `float` | 必填 | `count` 的公开输入或数据字段。 |
| `start_index` | `int | None` | `None` | `start_index` 的公开输入或数据字段。 |
| `end_index` | `int | None` | `None` | `end_index` 的公开输入或数据字段。 |
