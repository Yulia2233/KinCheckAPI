...
```python
summarize_energy(*, history: 'DynamicsLoadHistory') -> kincheckapi.physics_types.PhysicsReport
```
...
执行公开操作 `summarize_energy`。

## 参数与字段

| 名称 | 类型 | 默认值 | 说明 |
| --- | --- | --- | --- |
| `history` | `DynamicsLoadHistory` | 必填 | `history` 的公开输入或数据字段。 |
