...
| [`ProfileBoundary`](ProfileBoundary.md) | 枚举 | 定义 `ProfileBoundary` 接受的稳定枚举值。 |
...
| [`MotionSegment`](MotionSegment.md) | 类型 | 表示 `MotionSegment` 的公开、可序列化数据结构。 |
...
| [`remove_joint_driver`](remove_joint_driver.md) | 函数 | 执行公开操作 `remove_joint_driver`。 |
| [`clear_joint_drivers`](clear_joint_drivers.md) | 函数 | 执行公开操作 `clear_joint_drivers`。 |
...
| [`set_initial_state_from_home`](set_initial_state_from_home.md) | 函数 | 设置字段并返回更新后的不可变对象：`set_initial_state_from_home`。 |
| [`reset_to_home`](reset_to_home.md) | 函数 | 执行公开操作 `reset_to_home`。 |
...
| [`set_profile_boundary`](set_profile_boundary.md) | 函数 | 设置字段并返回更新后的不可变对象：`set_profile_boundary`。 |
